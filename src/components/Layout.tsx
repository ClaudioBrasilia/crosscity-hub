...
const Layout = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    setShowOnboarding(onboardingStatus === 'pending');
  }, [user]);
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" data-tour="menu-button">
              ...
              {primaryNavItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    data-tour={item.path === '/wod' ? 'wod-nav' : item.path === '/battle' ? 'duelos-nav' : item.path === '/leaderboard' ? 'ranking-nav' : undefined}
