...
const Leaderboard = () => {
  ...
  const loadData = useCallback(async () => {
    const [allUsers, allCheckins] = await Promise.all([
      getAllUsers(),
      db.getAllCheckins(),
    ]);
    setUsers(allUsers);
  ...
  const rankingFor = (category: Category, gender: Gender) => {
    return [...users]
      .filter(user => (user.category || 'beginner') === category && (user.gender || 'male') === gender)
          ...
          <Card key={user.id} className={`border-primary/20 ${index < 3 ? 'bg-primary/10' : ''}`}>
            <CardContent className="p-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{genderLabels[user.gender as Gender]}</p>
              </div>
        ...
        {(['rx', 'scaled', 'beginner'] as Category[]).map((category) => (
          <TabsContent key={category} value={category}>
            <Card className="border-primary/20">
              <CardHeader><CardTitle>{categoryLabels[category]} — Ranking por Gênero</CardTitle></CardHeader>
              <CardContent>
              ...
              {frequencyRanking.map((user, index) => (
                <Card key={user.id} className={`border-primary/20 ${index < 3 ? 'bg-primary/10' : ''}`}>
                  <CardContent className="p-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold truncate">{user.name}</p>
                      <p className="text-xs text-muted-foreground">{categoryLabels[user.category as Category] || 'Iniciante'} • {genderLabels[user.gender as Gender] || 'Masculino'}</p>
                    </div>
